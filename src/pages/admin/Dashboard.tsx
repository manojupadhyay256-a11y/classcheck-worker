...
const AdminDashboard = () => {
    ...
    const categories = [
        {
            ...
            icon: School,
            actions: [
                { title: 'Teachers', desc: 'Faculty & staff profiles', icon: Users, color: 'bg-amber-50', iconColor: 'text-amber-600', path: '/admin/teachers' },
                { title: 'Classes', desc: 'Set up class & sections', icon: School, color: 'bg-amber-50', iconColor: 'text-amber-600', path: '/admin/classes' },
                { title: 'Students', desc: 'Manage student records', icon: UserPlus, color: 'bg-amber-50', iconColor: 'text-amber-600', path: '/admin/students' },
                { title: 'Subjects', desc: 'Assign roles to teachers', icon: BookOpen, color: 'bg-amber-50', iconColor: 'text-amber-600', path: '/admin/subject-assignments' }
            ]
        },
        {
            ...
            actions: [
                { title: 'Analytics', desc: 'Daily activity & logs', icon: Activity, color: 'bg-amber-50', iconColor: 'text-amber-600', path: '/admin/log-book' },
                { title: 'Syllabus', desc: 'Academic progression', icon: BookOpen, color: 'bg-amber-50', iconColor: 'text-amber-600', path: '/admin/syllabus' },
                { title: 'Notifications', desc: 'Broadcast to everyone', icon: Bell, color: 'bg-amber-50', iconColor: 'text-amber-600', path: '/admin/notifications' },
    ...
    return (
        <div className="space-y-12 pb-12 overflow-x-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {kpis.map((kpi, i) => (
                    <motion.div
                        ...
                        className="bg-white p-7 rounded-3xl soft-shadow border border-gray-100 flex flex-col gap-6 hover:shadow-2xl hover:shadow-amber-900/5 transition-all group"
                    >
                        <div className="flex justify-between items-start">
                            ...
                            {kpi.trend && (
                                <span className="text-[10px] font-semibold uppercase tracking-wider text-amber-600 bg-amber-50 px-3 py-1.5 rounded-full">
                                    {kpi.trend}
                                </span>
                            )}
                        </div>
                        <div>
                            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">{kpi.label}</p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            {category.actions.map((action, i) => (
                                <motion.div
                                    ...
                                    className="bg-white p-6 rounded-3xl soft-shadow border border-gray-50 hover:shadow-2xl hover:shadow-amber-900/10 cursor-pointer group transition-all"
                                >
                                    ...
                                    <div className="flex justify-between items-end">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-400 font-medium leading-tight">{action.desc}</p>
                                        </div>
                                        <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-amber-600 transition-colors">
            ...
            <div className="bg-amber-50 rounded-3xl p-8 border border-amber-100/50 relative overflow-hidden">
                {loading && (
                ...
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <h2 className="text-2xl font-black text-slate-900 tracking-tight">Today's Insights</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white p-6 rounded-2xl soft-shadow border border-white">
                        <div className="flex items-center gap-3 mb-4">
                            <Clock className="w-5 h-5 text-amber-500" />
                            <span className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Late Attendance</span>
                        </div>
                        <p className="text-3xl font-extrabold text-slate-900 tabular-nums">{metrics.lateAttendance} Records</p>
                    ...
                    <div className="bg-white p-6 rounded-2xl soft-shadow border border-white">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Syllabus Completion</span>
                        </div>
                        <p className="text-3xl font-extrabold text-slate-900 tabular-nums">{metrics.syllabusProgress}% Overall</p>
                        <div className="w-full bg-gray-50 h-2 rounded-full mt-4 overflow-hidden">
                            <div className={`h-full bg-amber-600 rounded-full transition-all duration-1000`} style={{ width: `${metrics.syllabusProgress}%` }} />
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl soft-shadow border border-white">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Teacher Presence</span>
                        </div>
                        <p className="text-3xl font-extrabold text-slate-900 tabular-nums">{metrics.staffPresent} / {metrics.totalTeachers}</p>
