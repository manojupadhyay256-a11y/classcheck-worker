                        ...
                        >
                            ...
                            {view === opt.key && (
                                <motion.div
                                    ...
                                    className="absolute inset-0 bg-amber-600 rounded-lg shadow-lg shadow-amber-900/20"
                                    style={{ zIndex: -1 }}
                    ...
                    <AnimatePresence mode="wait">
                        {view === 'teacher' ? (
                            teacherWiseData.map((teacher, index) => (
                                ...
                                >
                                    ...
                                    <div className="p-6 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-amber-500/10 rounded-xl flex items-center justify-center text-amber-600 text-lg font-semibold">
                                                {teacher.name.charAt(0)}
                                            </div>
                                            <div>
                                                <h3 className="text-base font-semibold text-slate-900 leading-none">{teacher.name}</h3>
                                                <p className="text-xs text-slate-400 mt-1 font-mono">{teacher.email}</p>
                                            </div>
                                        </div>
                                        <span className="inline-flex items-center gap-1 px-3 py-1 bg-amber-50 text-amber-600 text-xs font-semibold rounded-lg">
                                    ...
                                    <div className="px-6 pb-6 flex-1">
                                        <div className="border border-saas-border rounded-xl overflow-hidden">
                                            ...
                                            {teacher.assignments.map((a, i) => (
                                                ...
                                                >
                                                    <span className="font-semibold text-slate-900">{a.class_name}</span>
                            ...
                            classWiseData.map((cls, index) => (
                                ...
                                >
                                    {/* Card header — white with left amber accent */}
                                    <div className="p-6 flex items-center gap-4 border-b border-saas-border relative">
                                        ...
                                        <div>
                                            ...
                                            <h3 className="text-lg font-semibold text-slate-900 leading-tight">{cls.name}</h3>
                                        </div>
                                    </div>
                                        ...
                                        {cls.assignments.map(a => (
                                            ...
                                            >
                                                <div className="min-w-0">
                                                    <p className="text-xs font-semibold text-amber-600 mb-1">{a.subject_name}</p>
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className={`text-sm ${a.teacher_name ? 'text-slate-900 font-medium' : 'text-rose-500 italic text-xs'}`}>
                                                ...
                                                <button
                                                    ...
                                                    className="px-4 py-2 rounded-lg text-xs font-medium text-slate-500 bg-slate-100 hover:bg-amber-600 hover:text-white transition-all duration-200 active:scale-95 cursor-pointer min-h-[36px] whitespace-nowrap"
                                                >
                                                    Change
                                                </button>
                                            </div>
            ...
            <AnimatePresence>
                {isReassignModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            {/* Amber accent bar */}
                            <div className="h-1 bg-amber-600 w-full shrink-0" />

                            {/* Modal header */}
                            <div className="p-6 border-b border-saas-border shrink-0">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h3 className="text-lg font-semibold text-slate-900">Assign Teacher</h3>
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="px-2.5 py-1 bg-amber-500/10 text-amber-600 text-xs font-semibold rounded-md">
                                ...
                                <div className="relative mt-4">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-saas-border rounded-xl text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/30 transition-all duration-200 search-inset min-h-[44px]"
                                    />
                                </div>
                            </div>
                                    ...
                                    .map(teacher => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 bg-amber-500/10 rounded-lg flex items-center justify-center text-amber-600 text-sm font-semibold group-hover:bg-amber-600 group-hover:text-white transition-all duration-200">
                                                    {teacher.name.charAt(0)}
                                                </div>
                                                <div className="text-left">
                                                    <p className="text-sm font-semibold text-slate-900 group-hover:text-amber-600 transition-colors duration-200">{teacher.name}</p>
                                            ...
                                            {reassignLoading && selectedAssignment?.teacher_id === teacher.id ? (
                                                ...
                                                <div className="w-9 h-9 rounded-lg bg-slate-100 flex items-center justify-center group-hover:bg-amber-500/10 transition-all duration-200">
                                                    <UserPlus className="w-4 h-4 text-slate-400 group-hover:text-amber-600 transition-colors duration-200" strokeWidth={2} />
                                                </div>
                                            )}
                                        </button>
                                    ))}
